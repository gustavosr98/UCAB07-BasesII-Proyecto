CREATE OR REPLACE PROCEDURE reporte13 (cursor13 OUT SYS_REFCURSOR, fechaInicio IN VARCHAR2, fechaFin IN VARCHAR2)
IS
    foto BLOB;
    fecha VARCHAR2(100);
    lOrigen VARCHAR2(50);
...
BEGIN
    ...
    SELECT aseg.logo, TIEMPO_PKG.PRINT(fechaInicio, 'FECHA_MM') || ' - ' || TIEMPO_PKG.PRINT(fechaFin, 'FECHA_MM'), 
        ...
        INTO foto, fecha, lOrigen, lDestino, cantidad
    FROM (
...
BEGIN
    ...
    IF (rvuelo.v_es_ida_vuelta = 'T') THEN
        SELECT getLugar(lDestino.fk_lugar, 'CIUDAD') INTO vueloRegreso
        FROM (SELECT MAX(v.periodo_estimado.fecha_fin) as rvueloRegreso, lDestino.id as lug --MIN O MAX??? 
